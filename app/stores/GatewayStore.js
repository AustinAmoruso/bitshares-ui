        ...
        if (coins && bridgeCoins && wallets) {
            ...
            bridgeCoins = bridgeCoins.filter(a => {
                return a && coins_by_type[a.outputCoinType] && (
                    (// Only use bitshares2 wallet types
                    coins_by_type[a.outputCoinType].walletType === "bitshares2" && this.bridgeInputs.indexOf(a.inputCoinType) !== -1) // Only use coin types defined in bridgeInputs
                );
            }).forEach(coin => {
