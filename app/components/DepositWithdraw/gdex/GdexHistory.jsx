...
class GdexHistory extends React.Component {
    ...
    componentWillReceiveProps(nextProps){
        if(this.props.recordType != nextProps.recordType || this.props.assetId != nextProps.assetId ||
        this.props.assetName!= nextProps.assetName || this.props.userId != nextProps.userId ||
    ...
    _getTransactionHistory(userId, assetId, assetName, recordType, userAccount, force=false){
        let pageSize = this.props.pageSize;
        let {pageNum, allData, historyStatusValue, historyStatusType} = this.state;
        if(force==true) {
            pageNum=1;
            allData={};
        ...
        var _this = this;
        getTransactionRecordList({"uid":userId, "assetId":assetId, "pageNum": pageNum,
            "pageSize": pageSize, "userAccount": userAccount}, recordType).then(res =>{
                if(res.records){
                    res.records.forEach(item =>{
                        allData[item.txid] = {"txid": item.txid, "amount": item.amount,
                            "time": item.ctime, "unixtime":Date.parse(item.ctime), "status": ["gateway", historyStatusType, historyStatusValue[item.status]].join("."), "failReason":item.failReason, "assetName": assetName};
                    });
    ...
    render() {
        ...
        let display_history = history.length ?
            history.map(o => {
                return (
                    <tr key={o.txid}>
                        <td style={{textAlign: "left"}} className="left-td column-hide-tiny"><Translate content={o.status}></Translate></td>
                        <td style={{padding: "8px 5px", textAlign: "left", fontSize: 14}}><Translate content="gateway.transaction_history_info" amount={o.amount} assetName = {assetName} txid={o.txid}   /></td>
                        <td>{o.time}</td>
                    </tr>
                );
                        ...
                        <table className={"table" + (compactView ? " compact" : "") + (this.props.dashboard ? " dashboard-table" : "")}>
                            <thead>
                                <tr key="history_header">
                                    <th><Translate content="gateway.status" /></th>
                                    <th><Translate content="gateway.info" /></th>
                                    <th><Translate content="gateway.time" /></th>
                                </tr>
                            </thead>
